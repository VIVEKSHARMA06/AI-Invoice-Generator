...
const Navbar = () => {
        ...
        try {
            ...
            if (token) {
                try {
                    ...
                    // ignore all errors in token generation
                }
                return token;
            } else {
                return null;
    ...
    useEffect(() => {
        ...
        (async () => {
            if (isSignedIn) {
                ...
                if (!t && mounted) {
                    await fetchAndStoreToken({ forceRefresh: true }).catch(
                        () => null,
                    );
                }
            } else {
    ...
    function openSignIn() {
        try {
            ...
            console.log("OpenSignin failed", error);
            navigate("/login");
        }
    }
    ...
    function openSignUp() {
        try {
            ...
            console.log("OpenSignup failed", error);
            navigate("/signup");
        }
    }
                    ...
                    <div className="flex items-center gap-4">
                        <div className={navbarStyles.authSection}>
                            <Show when="signed-out">
                                ...
                                >
                                    <div
                                        className={navbarStyles.signUpOverlay}
                                    ></div>
