...
// Function to build the invoice prompt
...
// it will generate invoice JSON based on the prompt provided by the user
    ...
    try {
        if (!API_KEY) {
            return res.status(500).json({
                success: false,
                message: "Missing GEMINI API key in environment variables",
            });
        }
        ...
        console.error("AI Invoice generation errror:", error);
        return res.status(500).json({
            success: false,
            message: "AI generation failed",
